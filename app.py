...
def build_scene_and_sensor(
    ...
    # ── LiDAR 2D — Slamtec S2E ────────────────────────────────────────────────
    # Especificaciones de referencia:
    #   32 000 muestras/s ÷ 10 Hz = 3 200 haces/vuelta  →  0,1125° resolución angular
    #   Alcance 90% reflectividad: 0,05 m – 30 m
    ...
    origin_x_m    = st.sidebar.slider(
        "LiDAR X [m]", -0.10, float(scene.capture_x_m + 0.20),
    ...
    num_beams     = st.sidebar.slider(
        "Número de haces  (S2E: 3 200 @ 10 Hz)",
        min_value=60,
        max_value=3200,    # 32 000 pts/s ÷ 10 Hz
        value=3200,
        step=30,
    )
    max_range_m   = st.sidebar.slider(
        "Alcance máximo [m]  (S2E: 30 m @ 90%)",
        min_value=0.05,    # rango ciego real del S2E
        max_value=30.00,   # 90% reflectividad
        value=30.00,
        step=0.05,
    )
    noise_std_m   = st.sidebar.slider(
        "Ruido gaussiano [m]  (S2E: ±30 mm)",
        min_value=0.000,
        max_value=0.050,   # holgura sobre los ±30 mm spec
        value=0.030,       # precisión nominal S2E
        step=0.001,
    )
